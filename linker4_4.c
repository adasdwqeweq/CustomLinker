 ...
 */
    ...
    case R_MIPS_REL32:
        ...
        case R_386_32:
            ...
            DL_ERR("RELO R_386_32 %08x <- +%08x %s", reloc, sym_addr, sym_name);
            *((Elf32_Addr*)reloc) += sym_addr;
            break;
        ...
        case R_386_PC32:
            count_relocation(kRelocRelative);
            MARK(rel->r_offset);
            DL_ERR("RELO R_386_PC32 %08x <- +%08x (%08x - %08x) %s",
...
static int LoadSegments4_4(int fd_,const char *name_,const Elf32_Phdr* phdr_table_,size_t phdr_num_,void* load_start_,Elf32_Addr load_size_,Elf32_Addr load_bias_) {
  ...
  for(i = 0; i < phdr_num_; ++i) {
    ...
    if (file_length != 0) {
      ...
      DL_INFO("LoadSegments seg_addr=%0x  flag=%08x", (unsigned)seg_addr,PFLAGS_TO_PROT(phdr->p_flags));
